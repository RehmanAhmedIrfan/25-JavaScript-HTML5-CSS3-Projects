...
document.addEventListener("DOMContentLoaded", function () {
    // When the DOM is loaded, call the function 
    // to initialize the unit dropdowns with the default type "length"
    updateUnitsDropdown("length");
});
// Function to update the unit dropdowns 
// based on the selected conversion type
function updateUnitsDropdown(type) {
    // Define units for various conversion types in an object
    const units = {
        length: ['meter', 'kilometer', 'centimeter', 'mile', 'yard'],
        area: ['square meter', 'square kilometer', 'square mile', 'square yard', 'square foot'],
        weight: ['kilogram', 'gram', 'ounce', 'pound', 'ton'],
        volume: ['cubic meter', 'cubic kilometer', 'cubic centimeter', 'liter', 'milliliter'],
...
function convert() {
    ...
    const conversionFactors = {
        length: { // Conversion factors for length units
            meter: {
        ...
        area: { // Conversion factors for area units
            'square meter': {
                'square kilometer': value => value / 1e+6,
                'square mile': value => value / 2.59e+6,
                'square yard': value => value * 1.19599,
                'square foot': value => value * 10.764
            },
            'square kilometer': {
                'square meter': value => value * 1e+6,
                'square mile': value => value / 2.59,
                'square yard': value => value * 1.196e+9,
                'square foot': value => value * 1.076e+7
            },
            'square mile': {
                'square meter': value => value * 2.59e+6,
                'square kilometer': value => value * 2.59,
                'square yard': value => value * 3.098e+6,
                'square foot': value => value * 2.788e+7
            },
            'square yard': {
                'square meter': value => value / 1.196,
                'square kilometer': value => value / 1.196e+9,
                'square mile': value => value / 3.098e+6,
                'square foot': value => value * 9
            },
            'square foot': {
                'square meter': value => value / 10.764,
                'square kilometer': value => value / 1.076e+7,
                'square mile': value => value / 2.788e+7,
                'square yard': value => value / 9
            },
        },
        weight: { // Conversion factors for weight units
            kilogram: {
        ...
        volume: { // Conversion factors for volume units
            'cubic meter': {
                'cubic kilometer': value => value / 1e+9,
                'cubic centimeter': value => value * 1e+6,
                liter: value => value * 1000,
                milliliter: value => value * 1e+6
            },
            'cubic kilometer': {
                'cubic meter': value => value * 1e+9,
                'cubic centimeter': value => value * 1e+15,
                liter: value => value * 1e+12,
                milliliter: value => value * 1e+15
            },
            'cubic centimeter': {
                'cubic meter': value => value / 1e+6,
                'cubic kilometer': value => value / 1e+15,
                liter: value => value / 1000,
                milliliter: value => value / 1e+6
            },
            liter: {
                'cubic meter': value => value / 1000,
                'cubic kilometer': value => value / 1e+12,
        ...
        temperature: { // Conversion factors for temperature units
            Celsius: {
        ...
        time: { // Conversion factors for time units
            second: {
